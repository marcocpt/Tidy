...
在进入功能开发前，先完成技术验证与项目骨架，不交付面向用户的产品功能：

- AX 权限检测、窗口枚举、AXSetFrame 兼容性、AX 聚焦；
- Carbon 热键注册、CGEventTap 生命周期；
- NSPanel 覆盖层、NSStatusItem 状态栏；
- 自适应网格算法、多显示器坐标、排列完成时间；
- SPM 三 target 骨架（TidyCore / TidyUI / TidyApp）可编译；
- `.swiftlint.yml` 配置并通过 `--strict`。

详细范围、FR、NFR、验收标准见 [phases/P0_技术探针/P0_01_阶段需求与验收.md](phases/P0_技术探针/P0_01_阶段需求与验收.md)；模块划分、数据流、状态机、错误回退见 [phases/P0_技术探针/P0_02_设计文档.md](phases/P0_技术探针/P0_02_设计文档.md)。P0 Exit Gate 通过后才开始 F0 功能开发。
...
| F5_跨App编排 | 多 App 窗口同时编排 | 跨 App 编排可用 |
