...
基于 Tidy spec 的 7 任务实现计划：

| 阶段 | 任务 | 交付物 |
|------|------|--------|
| 基础设施 | 1. SPM 三 target 结构、状态栏入口与 CI | TidyCore/TidyUI/TidyApp 骨架 + 状态栏图标 + CI |
| | 2. 偏好设置窗口（SwiftUI）与权限引导 | 4 标签页偏好窗口 + 权限引导窗口 |
| 核心交互 | 3. 窗口枚举与 frame 快照（F1.1） | 控制台输出窗口列表 + 快照验证 |
| | 4. 自适应网格布局与 AX 设置 frame（F1.2） | 多窗口被排列到目标屏幕网格内 |
| | 5. 字母标签覆盖层与选择输入（F1.3） | 字母标签可见 + 按字母有效 |
| | 6. 选中窗口最大化与 working 状态（F1.4） | 字母选择后窗口最大化 + 菜单栏锁标识 |
| 收口 | 7. 还原流程、异常清理与偏好驱动（F1.5 + F1.6） | 还原成功 + 异常退出 + 5 项配置生效 |
...
> v1.0 单 App 多窗口编排 → v1.5 应用专属配置与自定义布局 → v2.0 循环切换与跨 App 编排
