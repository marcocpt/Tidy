...
P0 探针按职责拆分为以下模块。模块划分参考功能列表 M1~M5 的分层，但 P0 探针阶段允许更扁平的组织以加速验证。模块名采用描述性命名，P0 期间允许与最终生产代码名不同；F1 阶段会基于验证结果正式化（见第 8 节迁移路径）。
...
- **被依赖**：FrameSnapshotStore（还原时调用）、编排协调器（排列与最大化时调用）。

#### FrameSnapshotStore

- **职责**：保存排列前的窗口 frame 快照；按快照还原所有窗口；跳过已关闭窗口；上报还原失败给编排协调器。
- **不负责**：不决定何时保存与还原（由编排协调器决定）；不持久化到磁盘（P0 仅内存）。
- **依赖**：WindowEnumerator（获取窗口引用与原始 frame）、WindowManipulator（还原时设置 frame）。
- **被依赖**：编排协调器。
...
- **被依赖**：LayoutCalculator、OverlayPanel、编排协调器。
...
- **职责**：创建 NSPanel 覆盖目标屏幕；渲染字母标签；淡入 / 淡出显示；不抢焦点、鼠标穿透；仅当前 Space 可见。
- **不负责**：不决定标签位置与字母（由 LabelAssigner 与 LayoutCalculator 决定）；不处理业务逻辑。
- **依赖**：LayoutCalculator（窗口新 frame 用于标签定位）、LabelAssigner（字母分配）、TargetScreenResolver（目标屏幕）。
- **被依赖**：编排协调器。
...
- **被依赖**：WindowManipulator（设置目标 frame）、OverlayPanel（标签定位）。
...
- **职责**：串联完整数据流（热键 → 枚举 → 快照 → 布局 → 设置 frame → 覆盖层 → 选择 → 最大化 → 还原）；维护简化 4 状态机；处理错误与降级。
- **不负责**：不实现具体能力（委托各模块）；不持久化会话。
...
| 前台 App 切换处理 | P0 探针不强制处理 | 自动还原到 `restoring` |
...
P0 必须建立可编译的 SPM 三 target 骨架（对应"依赖方向"ADR 主题）。三 target 划分以 [docs.md](../../docs.md) 第 5 节为准。

### 6.1 Target 职责划分

| Target | 类型 | 职责 | 依赖 | 包含的 P0 模块 |
|--------|------|------|------|----------------|
| `TidyCore` | Library | 纯 Swift + 系统框架，无 AppKit；承载窗口枚举、AX 操作、布局算法、状态机、屏幕信息协议 | 系统框架（CoreGraphics、ApplicationServices、Carbon） | PermissionDetector、WindowEnumerator、WindowManipulator、FrameSnapshotStore、TargetScreenResolver、LayoutCalculator、LabelAssigner、HotkeyRegistrar、GlobalEventTap、屏幕信息协议、编排协调器 |
| `TidyUI` | Library | AppKit 覆盖层 + SwiftUI 偏好窗口 + TidyCore | AppKit、SwiftUI、TidyCore | OverlayPanel、StatusItem、屏幕信息实现（基于 NSScreen） |
| `TidyApp` | Executable | 应用入口、生命周期、依赖注入 | TidyCore、TidyUI | 应用入口、依赖注入组装 |
...
- `TidyApp` 依赖 `TidyCore` 与 `TidyUI`，是唯一可执行 target；
...
2. **新增自动还原触发**：F1 监听前台 App 切换与 Space 切换，自动还原到 `restoring`；
