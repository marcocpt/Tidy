...
## 6. 项目骨架设计（Xcode App Target + SPM Libraries）

P0 必须建立可编译的项目骨架（对应"依赖方向"ADR 主题）。`TidyCore` / `TidyUI` 为 SPM Library Target，`TidyApp` / `TidyUITests` 为 Xcode Native Target，由 `Tidy.xcodeproj` 管理。Target 划分以 [docs.md](../../docs.md) 第 5 节 Standards 和本节为准。
...
| `TidyUITests` | XCUITest Target（Xcode Native Target） | App 启动 smoke test 等可稳定自动化的 UI 行为验证 | TidyApp（Test Host） | XCUITest 测试用例 |
...
>
> **入口点实现**：TidyApp 使用 `main.swift` 显式创建 `NSApplication.shared`、设置 `TidyAppDelegate` 为 delegate、调用 `app.run()`。原因：`@main` + `NSApplicationDelegate` 在无 main nib/storyboard 时不会自动设置 delegate，导致 `applicationDidFinishLaunching` 不被调用。`Info.plist` 设置 `LSUIElement=true` 使 App 作为 Menu Bar App 运行（不显示 Dock 图标）。
>
> **UI 测试分层**（对应 [P0_03 测试用例表](P0_03_测试用例表.md)）：
> - **SPM XCTest**：LayoutCalculator、LabelAssigner、快照/状态逻辑、窗口匹配算法等纯逻辑测试；
> - **XCUITest**：App 启动 smoke test（验证 App 不崩溃启动）等可稳定自动化的 UI 行为；
> - **手动验证 + artifacts/**：StatusItem 菜单（平台限制：XCUITest 无法访问 `NSStatusItem`）、Overlay 出现/消失、Esc/选择流程、跨 Space、窗口层级、鼠标穿透、多显示器等环境敏感行为，保留手动验证清单与证据，见 [artifacts/manual-verification-checklist.md](artifacts/manual-verification-checklist.md)。
>
> **平台限制说明**：XCUITest 在 macOS 上无法访问 `NSStatusItem`（系统状态栏不在 App 的 accessibility hierarchy 中），因此 AC-P0-008（状态栏可用）的图标显示、菜单点击、菜单项回调验证必须手动执行。Overlay 与 Esc 流程因依赖编排触发（需 AX 权限 + ≥2 可见窗口），在 XCUITest 环境下不稳定，亦降级为手动验证。P0 阶段不引入测试钩子（test hook）以避免过度工程化；F1 阶段若需稳定自动化可评估测试模式方案。
...
- **v0.7（2026-07-29）**
  - 第 6.1 节新增"入口点实现"说明：TidyApp 使用 `main.swift` 显式设置 NSApplication delegate（`@main` + `NSApplicationDelegate` 在无 main nib 时不会自动设置 delegate），`Info.plist` 设置 `LSUIElement=true` 使 App 作为 Menu Bar App 运行；
  - 变更来源于 MV-P0-008 手动验证发现的根因：状态栏图标不显示，原因是 `applicationDidFinishLaunching` 未被调用。

- **v0.6（2026-07-29）**
  - 第 6.1 节 UI 测试分层修正：基于 XCUITest 实测平台限制（无法访问 `NSStatusItem`），将 StatusItem 菜单、Overlay 出现/消失、Esc/选择流程从 XCUITest 层移至手动验证层；
  - 第 6.1 节新增平台限制说明，明确 AC-P0-008 必须手动验证，P0 不引入测试钩子；
  - `TidyUITests` Target 职责描述调整为"App 启动 smoke test 等可稳定自动化的 UI 行为验证"；
  - 手动验证清单整合到 `artifacts/manual-verification-checklist.md`。

- **v0.5（2026-07-29）**
  - 第 6 节标题从"SPM 三 target 骨架设计"改为"项目骨架设计（Xcode App Target + SPM Libraries）"，反映混合结构；
  - 第 6.1 节 Target 职责划分表新增 `TidyUITests` XCUITest Target 行，明确其职责为可自动化的 UI 行为验证；
  - 第 6.1 节新增 UI 测试分层说明：SPM XCTest（纯逻辑）/ XCUITest（可自动化 UI）/ 手动验证（环境敏感行为）三层，对应 P0_03 测试用例表。
